...
class BasicBot extends TelegramBot with Polling with Commands[Future] with Callbacks[Future] with ChatActions[Future]:
  ...
  /**
    ...
    */
     ...
     */
    def sendPhoto(filename: String, selectedChatId: ChatId, caption: Option[String] = None): Unit = {
        val photo = InputFile(java.nio.file.Paths.get(filename))
        caption match
         case None => request(SendPhoto(selectedChatId, photo))
         case Some(c) => request(SendPhoto(selectedChatId, photo, caption))
  ...
  /**
    ...
    * @param selectedChatId Chat id where to send the image
    */
    def sendBufferedImage(image: BufferedImage, selectedChatId: ChatId, caption: Option[String] = None): Unit = {
        ...
        caption match
          case None => request(SendPhoto(selectedChatId, contents))
          case Some(c) => request(SendPhoto(selectedChatId, contents, caption))
    }
